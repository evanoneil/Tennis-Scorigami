...
if("minutes" %in% colnames(atp_clean) && "surface" %in% colnames(atp_clean)) {
  ...
  if(nrow(surface_with_duration) > 1) {
    ...
    if(anova_summary[[1]][["Pr(>F)"]][1] < 0.05) {
      ...
      # Format Tukey results as a table
...
## First Serve % and Match Outcome
```{r first-serve-analysis}
# Analysis of first serve percentage and match outcome
if("winner_1st_serve_pct" %in% colnames(atp_clean) && "loser_1st_serve_pct" %in% colnames(atp_clean)) {
  # Create a dataset for analysis
  serve_data <- atp_clean %>%
    filter(!is.na(winner_1st_serve_pct) & !is.na(loser_1st_serve_pct)) %>%
    mutate(
      winner_had_better_1st_serve = winner_1st_serve_pct > loser_1st_serve_pct,
      serve_diff = winner_1st_serve_pct - loser_1st_serve_pct
    )
  
  # Calculate how often the player with better first serve % wins
  better_serve_wins <- serve_data %>%
    summarise(
      total_matches = n(),
      better_serve_winner_count = sum(winner_had_better_1st_serve),
      better_serve_winner_pct = round(100 * sum(winner_had_better_1st_serve) / n(), 1)
    )
  
  # Display results
  datatable(better_serve_wins,
            options = list(dom = 't', ordering = FALSE),
            caption = "How Often Does the Player with Better 1st Serve % Win?") %>%
    formatRound(columns = "better_serve_winner_pct", digits = 1)
  ...
  ggplot(serve_data, aes(x = serve_diff)) +
    ...
    labs(
      title = "Difference in 1st Serve % Between Winners and Losers",
      x = "Winner's 1st Serve % - Loser's 1st Serve %",
      y = "Count"
    ) +
    theme_minimal()
  
  # Statistical test - is the difference significant?
  t_test_result <- t.test(serve_data$winner_1st_serve_pct, serve_data$loser_1st_serve_pct, paired = TRUE)
  
  # Format t-test results
  t_test_df <- data.frame(
    Statistic = c("t-value", "df", "p-value", "Mean Difference", "95% CI Lower", "95% CI Upper"),
    Value = c(
      t_test_result$statistic,
      t_test_result$parameter,
      t_test_result$p.value,
      t_test_result$estimate[1] - t_test_result$estimate[2],  # Correct way to calculate mean difference
  ...
  datatable(t_test_df,
            options = list(dom = 't', ordering = FALSE),
            caption = "Paired t-test: Winner vs. Loser 1st Serve %") %>%
...
# Analysis of left-handers vs. right-handers
if("winner_hand" %in% colnames(atp_clean) && "loser_hand" %in% colnames(atp_clean)) {
  # Create a dataset for analysis
  handedness_data <- atp_clean %>%
    filter(!is.na(winner_hand) & !is.na(loser_hand)) %>%
    mutate(
      match_type = case_when(
        winner_hand == "L" & loser_hand == "R" ~ "Left beats Right",
        winner_hand == "R" & loser_hand == "L" ~ "Right beats Left",
        winner_hand == "L" & loser_hand == "L" ~ "Left beats Left",
        winner_hand == "R" & loser_hand == "R" ~ "Right beats Right",
        TRUE ~ "Other"
      )
    )
  
  # Count matches by handedness combination
  handedness_counts <- handedness_data %>%
    count(match_type) %>%
    arrange(desc(n))
  
  # Display results
  datatable(handedness_counts,
            options = list(dom = 't', ordering = FALSE),
            caption = "Match Outcomes by Player Handedness")
  
  # Calculate win rates for left vs right matchups
  left_vs_right <- handedness_data %>%
    filter(winner_hand %in% c("L", "R") & loser_hand %in% c("L", "R")) %>%
    summarise(
      left_vs_right_matches = sum(winner_hand == "L" & loser_hand == "R") + 
                              sum(winner_hand == "R" & loser_hand == "L"),
      left_beats_right = sum(winner_hand == "L" & loser_hand == "R"),
      left_beats_right_pct = round(100 * sum(winner_hand == "L" & loser_hand == "R") / 
                                  (sum(winner_hand == "L" & loser_hand == "R") + 
                                   sum(winner_hand == "R" & loser_hand == "L")), 1)
    )
  
  # Display left vs right results
  datatable(left_vs_right,
            options = list(dom = 't', ordering = FALSE),
            caption = "Left-Handed vs. Right-Handed Player Matchups") %>%
    formatRound(columns = "left_beats_right_pct", digits = 1)
  
  # Chi-square test for independence
  # Create contingency table
  handedness_table <- table(handedness_data$winner_hand, handedness_data$loser_hand)
  
  # Run chi-square test
  chi_test <- chisq.test(handedness_table)
  
  # Format chi-square results
  chi_test_df <- data.frame(
    Statistic = c("Chi-square", "df", "p-value"),
    Value = c(chi_test$statistic, chi_test$parameter, chi_test$p.value)
  )
  
  # Display chi-square results
  datatable(chi_test_df,
            options = list(dom = 't', ordering = FALSE),
            caption = "Chi-square Test: Winner Handedness vs. Loser Handedness") %>%
    formatRound(columns = "Value", digits = 4)
}
```

## Racquet Brand Analysis
```{r racquet-brand-analysis}
# Load the tennis sponsorships data
sponsorships <- read.csv("tennis-sponsorships.csv", stringsAsFactors = FALSE)

# Clean and prepare the sponsorships data
sponsorships <- sponsorships %>%
  mutate(
    player_name = paste(First.Name, Last.Name),
    racquet_brand = Racket
  ) %>%
  select(player_name, racquet_brand, Country)

# Merge with match data
if(exists("atp_clean")) {
  # Create a dataset with all player appearances
  all_players <- rbind(
    atp_clean %>% 
      select(player_name = winner_name, result = "win"),
    atp_clean %>% 
      select(player_name = loser_name, result = "loss")
  )
  
  # Join with sponsorships data
  players_with_racquets <- all_players %>%
    left_join(sponsorships, by = "player_name") %>%
    filter(!is.na(racquet_brand))
  
  # Calculate win rates by racquet brand
  racquet_performance <- players_with_racquets %>%
    ...
    filter(total_matches >= 10) %>%  # Only include brands with sufficient data
    arrange(desc(win_rate))
  
  # Display results
  datatable(racquet_performance,
            options = list(dom = 't', pageLength = nrow(racquet_performance)),
            caption = "Performance by Racquet Brand") %>%
    formatRound(columns = "win_rate", digits = 1)
  
  # Visualize the top racquet brands by win rate
  top_racquets <- racquet_performance %>%
    filter(total_matches >= 20) %>%  # Only include brands with more data for visualization
    arrange(desc(win_rate)) %>%
    head(10)
  
  ggplot(top_racquets, aes(x = reorder(racquet_brand, win_rate), y = win_rate)) +
    geom_bar(stat = "identity", fill = "darkblue") +
    geom_text(aes(label = paste0(win_rate, "%")), hjust = -0.2) +
    labs(
      title = "Win Rate by Racquet Brand",
      subtitle = "Minimum 20 matches played",
      x = "Racquet Brand",
      y = "Win Rate (%)"
    ) +
    coord_flip() +
  ...
  # ANOVA test for racquet brand effect on win rate
  # Prepare data for ANOVA
  racquet_anova_data <- players_with_racquets %>%
    mutate(result_numeric = ifelse(result == "win", 1, 0)) %>%
    filter(racquet_brand %in% top_racquets$racquet_brand)
  
  # Run ANOVA
  racquet_anova <- aov(result_numeric ~ racquet_brand, data = racquet_anova_data)
  anova_summary <- summary(racquet_anova)
  
  # Format ANOVA results
  anova_table <- data.frame(
    Source = c("Racquet Brand", "Residuals"),
    Df = c(anova_summary[[1]][["Df"]][1], anova_summary[[1]][["Df"]][2]),
    Sum_Sq = c(anova_summary[[1]][["Sum Sq"]][1], anova_summary[[1]][["Sum Sq"]][2]),
    Mean_Sq = c(anova_summary[[1]][["Mean Sq"]][1], anova_summary[[1]][["Mean Sq"]][2]),
    F_value = c(anova_summary[[1]][["F value"]][1], NA),
  ...
  datatable(anova_table,
            options = list(dom = 't', ordering = FALSE),
            caption = "ANOVA: Effect of Racquet Brand on Win Rate") %>%
    formatRound(columns = c("Sum_Sq", "Mean_Sq", "F_value", "P_value"), digits = 4)
}
```
